...
class AuthorDetail extends Component {
  ...
  fetch_details() {
    const that = this;
    let listreq = {
      url: "/api/viewauthor/" + this.props.match.params.id,
      method: "get"
    };
    let authreq = {
      url: "/api/authorprofile/" + this.props.match.params.id,
      method: "get"
    };
    axios.all([axios.request(listreq), axios.request(authreq)]).then(
      axios.spread(function(res1, res2) {
        that.books = res1.data;
        that.author = res2.data[0];
        if (res2.data.length === 0) that.author = false;
  ...
  render() {
    if (this.state.loaded && !this.author) return <Redirect to="/authors" />;
      ...
      for (let book in this.books) {
        list.push(
          <div
            className="list-item"
            key={book}
            onClick={() => this.navigateTo(this.books[book].id)}
          >
            <img src={booklogo} alt="" />
            <div className="list-content">
              <div className="booklist-row1">
                <span className="book-name">{this.books[book].name}</span>
          ...
          <div className="author-details">
            <div className="list-header">
              ...
              <div className="authorlist-row2">
                <span>Age {this.author.age}</span> /<span> {gender}</span>
              </div>
              <div className="">
    ...
    return (
      <div className="component-container">
        ...
        <div className="container">
          <div className="content-heading">BOOKS</div>
